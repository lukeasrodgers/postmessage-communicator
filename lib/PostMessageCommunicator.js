(function() {
  ...
  PMC.prototype.create_listener = function() {
    ...
    add_listener(function(e) {
      ...
      if (data.instantiate) {
        // instantiate an object to interact with the remote receiver
        ...
        that.origin_whitelist = [e.origin];
        that.sender.remote_execute = function() {
          that.remote_execute.apply(that, arguments);
        };
        ...
        for (var i = 0, len = ctx_methods.length; i < len; i++) {
          fn_name = ctx_methods[i].match(/[^(this.)].*$/)[0];
          if (!has_func.call(that, fn_name)) {
            dynamic_sendables.push(fn_name);
          }
        }
        if (dynamic_sendables.length) {
          that.create_senders(dynamic_sendables);
        }
        var fn = eval(fn_body);
        var args = data.args;
        fn.apply(that.sender, args);
      }

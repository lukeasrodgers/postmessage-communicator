(function() {
  ...
  PMC.prototype.create_listener = function() {
    ...
    bound_listener = function() { return that.listener.apply(that, arguments); };
    add_listener(bound_listener);
  };

  PMC.prototype.listener = function(e) {
    var data = e.data;
    if (!this.whitelisted(e.origin)) {
      return;
    }
    try {
    ...
    if (data.instantiate) {
      this.instantiate_handler(e, data);
    }
    else if (data.recipient) {
      this.recipient = e.source;
      if (this.origin_whitelist) {
        this.target_origin = e.origin;
      }
    }
    else if (data.remote_execute) {
      this.remote_execute_handler(data);
    }
    else if (has_func.call(this, fn_name) && this.recipient) {
      this.sender[fn_name].apply(this.sender, fn_args);
    }
    else {
      console.warn('no match for', fn_name, fn_args, this.sender);
    }
  };
  ...
  PMC.prototype.remote_execute_handler = function(data) {
    ...
    // hack necessary to ensure this works in IE8
    var fn = eval('0, ' + fn_body);
    var args = data.args;
    fn.apply(this.sender, args);
  };

  PMC.prototype.dispose = function() {
    if (window.removeEventListener) {
      window.removeEventListener('message', bound_listener, false);
    }
    else {
      window.detachEvent('onmessage', bound_listener);
    }
  };
